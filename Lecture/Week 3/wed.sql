...
-- expression: the usual operators, plus lots fuctions like regex
...
-- we can alsk ask, how non-NULL values are there?
...
SELECT Location, COUNT(*) FROM Site GROUP BY Location; -- again, * is an idiom
...
-- The order of the caluses reflect the ordr of the processing
-- BUT, what if you want to do some filtering on your groups, i.e., *after* you've done the grouping?
SELECT locations, MAX(Area) AS Max_area
    FROM Site
...
-- You can save tables, you can next queries
...
-- What happens?
-- The result is the same as the table with the foreign, but augmented with additional columns
...
-- Table aliases
-- Sometimes, if column names are ambigious where they're coming from,
-- need qualify them
