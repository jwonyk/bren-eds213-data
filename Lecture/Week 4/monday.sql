...
-- Let's pretend that SQL didn't have HAVING clause. Could we somehow get the same functionality?
...
-- What is a join? Conceptually, the database performs a "Cartesian product" of the tables, then matches up rows based on some kind of join condition

-- In some databases, to do a Cartesian product you would jut do a JOIN without a condition, e.g.,
...
SELECT * FROM Student JOIN House USING (House_ID); -- Default to INNER when just used JOIN
...
SELECT Nest_ID, ANY_VALUE(Species), COUNT(*) -- ANY_VALUE: Aggregation, just return all the values?
