...
-- .table -- lists tables
-- .schema -- lists the whole shema
.schema

-- Getting help on SQL: look at the "railroad" diagrams in SQLite
-- Check out http://sqlite.org/lang.html
...
SELECT DISTINCT Species, Observer FROM Bird_nests LIMIT 3;
...
-- In-class challenge:
-- Select disctinct locations from the Site table; are they in order? If not, order them.
SELECT DISTINCT Location FROM SITE;
SELECT DISTINCT Location FROM SITE ORDER BY Location DESC;
SELECT DISTINCT Location FROM SITE ORDER BY Location ASC;